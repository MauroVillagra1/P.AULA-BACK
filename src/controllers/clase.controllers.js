...
export const crearClase = async (req, res) => {
  try {
    const { aula, profesor, comision, dia, horario_inicio, horario_fin } = req.body;
    if (!aula || !profesor || !comision || !dia || !horario_inicio || !horario_fin)
      return res.status(400).json({ error: "Todos los campos son obligatorios" });

    const nuevaClase = new Clase({ aula, profesor, comision, dia, horario_inicio, horario_fin });
...
export const actualizarClase = async (req, res) => {
  try {
    ...
    const clase = await Clase.findByIdAndUpdate(
      id,
      { aula, profesor, comision, dia, horario_inicio, horario_fin },
      { new: true }
    );
